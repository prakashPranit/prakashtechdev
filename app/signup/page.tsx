 ...
 * @param {string} type - The type of the form (default: "sign-up").
 * @returns {JSX.Element} - The rendered UserAuthForm component.
 */
const UserAuthForm = ({ type = "sign-up" }: { type?: string }): JSX.Element => {
  return (
    <AnimationWrapper keyValue={type}>
      <section className="h-cover flex items-center justify-center">
        <form className="w-[80%] max-w-[400px] ">
          ...
          {type !== "sign-in" ? (
            <InputBox name={"Fullname"} placeholder={"Fullname"} type={"text"} icon={"user"} />
          ) : (
            ""
          )}
          ...
          <button className="btn-dark center mt-14 flex items-center justify-center gap-4 w-[90%] center">
            <Image src={googleIcon} className="w-5"  alt="google"/> Continue with Google
          </button>
          {type === "sign-in" ? (
